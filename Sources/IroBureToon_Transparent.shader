Shader "IroBureToon/Transparent"
{
    Properties
    {
        [Header(Main)]
        ...
        _TransparentLevel ("Transparent Level", Range(0, 1)) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 fragBase(IBT_V2F i) : SV_Target
            {
                ...
                half3 color = IBT_BaseLighting(i,
                    irozureAlbedo,
                    normalWS,
                    lightDirection,
                    attenuation
        ...
        Pass
        {
            ...
            fixed4 fragAdd(IBT_V2F i) : SV_Target
            {
                ...
                contribution *= saturate(alpha * _AlphaBoostFA);
        ...
        Pass
        {
            ...
            CGPROGRAM
            #pragma vertex IBT_OutlineTransparentVert
            #pragma fragment IBT_OutlineTransparentFrag
            #pragma multi_compile_fog
            #pragma multi_compile_instancing
            #include "../Includes/IBT_Outline.cginc"

            struct IBT_OutlineTransparentV2F {
                float4 pos : SV_POSITION;
                float2 uv : TEXCOORD0;
                UNITY_FOG_COORDS(1) UNITY_VERTEX_OUTPUT_STEREO
            };

            IBT_OutlineTransparentV2F IBT_OutlineTransparentVert(IBT_AppData v)
            {
                IBT_OutlineV2F baseOutput = IBT_OutlineVert(v);
                IBT_OutlineTransparentV2F o;
                UNITY_INITIALIZE_OUTPUT(IBT_OutlineTransparentV2F, o);
            ...
            fixed4 IBT_OutlineTransparentFrag(IBT_OutlineTransparentV2F i) : SV_Target
            {
                half alpha = IBT_Alpha(i.uv);
                clip(alpha - _TransparentLevel);
